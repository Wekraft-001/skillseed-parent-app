...
import { 
  Download, BarChart3, Star, Lightbulb, Bell, Trophy, MessageSquare, 
  ArrowRight, Target, TrendingUp, ChevronRight
} from 'lucide-react';
import { PageMetadata } from '../components/PageMetadata';

const ReportsAndProgress = () => {
  ...
  return (
    <div className="bg-[#F5F7FA] min-h-[800px] p-6">
      ...
      <div id="child-selector" className="mb-8 flex items-center gap-4 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div id="progress-column" className="lg:col-span-2 space-y-6">
          {/* Progress Header */}
          <div id="progress-header" className="bg-white p-6 rounded-2xl shadow-sm relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {progressMetrics.map((metric) => (
                  <div key={metric.id} className={`${metric.bgColor} p-4 rounded-2xl relative overflow-hidden`}>
                    <div className="absolute top-0 right-0 w-8 h-8 rounded-full -mr-4 -mt-4" style={{ backgroundColor: `${metric.bgColor.replace('bg-', '').replace('-50', '-100')}` }}></div>
          ...
          {/* Interest Trends */}
          <div id="interest-trends" className="bg-white p-6 rounded-2xl shadow-sm">
            ...
            <div className="space-y-4">
              {interestTrends.map((trend) => (
                <div key={trend.id} className="flex items-center justify-between gap-4">
                  <span className="text-sm text-gray-600 w-24">{trend.name}</span>
                  ...
                  <span className="text-sm font-semibold text-gray-700 w-10 text-right">{trend.percentage}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div id="discussion-column" className="space-y-6">
          {/* Discussion Prompts */}
          <div id="discussion-prompts" className="bg-white p-6 rounded-2xl shadow-sm">
            <div className="flex items-center justify-between mb-6">
            ...
            <div className="space-y-3">
              {discussionPrompts.map((prompt) => (
                <div key={prompt.id} className={`${prompt.bgColor} p-4 rounded-xl flex items-start gap-3`}>
                  <div className={`p-2 rounded-lg ${prompt.bgColor.replace('50', '100')}`}>
                    {prompt.icon}
                  </div>
          ...
          {/* Recent Updates */}
          <div id="notifications" className="bg-white p-6 rounded-2xl shadow-sm">
            <div className="flex items-center justify-between mb-6">
            ...
            <div className="space-y-3">
              {recentUpdates.map((update) => (
                <div key={update.id} className={`${update.bgColor} p-4 rounded-xl flex items-start gap-3`}>
                  <div className={`p-2 rounded-full ${update.iconBg} flex-shrink-0`}>
                    {update.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-2">{update.timeAgo}</p>
                  </div>
                </div>
              ))}
            </div>
...
export default ReportsAndProgress;
